...
# ฟังก์ชันดึงความยาวไฟล์เสียงจริงเป็นวินาที
def get_audio_duration(file_path):
    cmd = [
        ffmpeg_bin, "-i", file_path
    ]
    result = subprocess.run(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE, text=True)
    # ค้นหาข้อความ Duration: 00:00:07.50 ใน log
    for line in result.stderr.split('\n'):
        if "Duration" in line:
            time_str = line.split("Duration:")[1].split(",")[0].strip()
            h, m, s = time_str.split(":")
            return float(h) * 3600 + float(m) * 60 + float(s)
    return 7.0 # ค่าเริ่มต้นสำรอง

# อ่านความยาวเสียงพากย์แต่ละไฟล์จริง
durations = [get_audio_duration(f'slides/audio_{i}.mp3') for i in range(1, 5)]
print("🔊 ความยาวเสียงจริงแต่ละสไลด์ (วินาที):", [round(d, 2) for d in durations])
...
.bar-bg {{ background-color: #0
cd ~/Desktop/reel_project

cat << 'EOF' > fix_slide3_audio.py
import os
import subprocess
import asyncio
import glob
...
.bar-fill {{ height: 100%; border-radius: 16px; width: 0%; animation-duration: 1.0s; animation-timing-function: cubic-bezier(0.16, 1, 0.3, 1); animation-fill-mode: forwards; }}
...
.b3 {{ animation-name: fill3; animation-delay: {durations[2]*0.5}s; background: linear-gradient(90deg, #c084fc, #e879f9); }}
...
async def process_slides():
    async with async_playwright() as p:
        ...
        await page3.goto(f"file://{os.path.abspath('slides/slide_{i}.html')}")
        await page3.wait_for_timeout(int((durations[2] + 2.0) * 1000))
        await context.close()
        await browser.close()
...
# 3. สร้างคลิปย่อยแบบขยายส่วนเผื่อเวลาจบ (Padding)
pad = 0.5 # เพิ่มเผื่อเวลาท้ายคลิปให้พูดจบก่อนเปลี่ยนหน้า
...
for i in [1, 2, 4]:
    cmd = [
        ...
        "-filter_complex", f"[0:v]trim=duration={durations[i-1]+pad}[v];[1:a]apad=pad_len=44100[a]",
        "-map", "[v]", "-map", "[a]",
...
cmd_part3 = [
    ...
    "-filter_complex", f"[0:v]trim=duration={durations[2]+pad}[v];[1:a]apad=pad_len=44100[a]",
    "-map", "[v]", "-map", "[a]",
...
# 4. คำนวณ Offset โดยอ้างอิงตามระยะความยาววิดีโอที่รวม Padding
trans = 0.3
...
print("🎯 แก้ไขตรงจุดแล้วครับ! เพิ่ม Margin บัฟเฟอร์ช่วงเปลี่ยนฉาก ทำให้เสียง Qualitative Chat Sentiment พูดจบครบใน Slide 3 พอดี ไม่โผล่ไป Slide 4 แล้วครับ")
