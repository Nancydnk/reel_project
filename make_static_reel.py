...
# 1. ลบ CSS Animations ออกทั้งหมด ให้เป็น Static Layout
s1 = """<!DOCTYPE html>
<html>
<head>
<style>
  body { margin: 0; background: #0B0F19; color: #fff; font-family: 'Segoe UI', system-ui, sans-serif; display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100vh; text-align: center; padding: 40px; box-sizing: border-box; }
  .badge { background: linear-gradient(135deg, #FF007A, #7928CA); padding: 12px 28px; border-radius: 30px; font-weight: 800; font-size: 24px; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 30px; }
...
<head>
<style>
  body { margin: 0; background: #0B0F19; color:
cd ~/Desktop/reel_project
cat << 'EOF' > make_static_reel.py
import os
import subprocess
import asyncio
import imageio_ffmpeg
...
print("🎥 กำลังสร้างวิดีโอแบบสไลด์นิ่งปกติ (ไม่มี Zoom / ไม่มี Fade)...")

# 3. รวมเป็นวิดีโอภาพนิ่งธรรมดา ตัดฉับตามความยาวเสียงพากย์
for i in range(1, 5):
    cmd = [
        ffmpeg_bin, "-y",
...
# รวม 4 ส่วนต่อกันตรงๆ ไม่มี Transition
...
print("\n🎉 เรียบร้อย! เอาเอฟเฟกต์การเคลื่อนไหวออกหมดแล้ว ได้ไฟล์ reel.mp4 แบบสไลด์นิ่งปกติเรียบร้อยครับ")
